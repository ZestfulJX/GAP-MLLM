...
def run_inference(rank, world_size, data_samples, pretrained_path, data_path, output_path):
    """Run inference on each device and keep outputs and IoU results in the original data order."""
    ...
    results = []  # Store records associated with the original index, output, and IoU.
    ...
    for i, (sample, original_idx) in enumerate(zip(local_samples, local_indices)):
        ...
        iou = ret.get('iou', None)
        
        # 3. Record the original index for later sorting, along with output and IoU.
        results.append({
    ...
    if rank == 0:
        ...
        aggregated = threedod_aggregate_results(final_results)
        ...
        # Compute average IoU.
        valid_ious = [
            item['threedod_score']['iou'] 
            for item in final_results 
            if item['threedod_score']['iou'] is not None 
            and isinstance(item['threedod_score']['iou'], (int, float))
        ]
        if valid_ious:
...
def threedod_aggregate_results(results):
    ...
    for result in results:
        # print(result)
        for k, v in result['threedod_score']["result"].items():
    ...
    all_metrics = {}
    table_data = [
        ["Category", "Precision", "Recall", "F1 Score"]  # Header.
    ]
...
def process_3d_video_object_detection(model, sample, data_path):
...
def main():
    ...
    with open(os.path.join("data/evaluation/threedod", "scannet_det_val_4frames.json")) as f:
        data = json.load(f)

    # with open(os.path.join("data/evaluation/threedod", "scannet_det_val_6frames.json")) as f:
    #     data = json.load(f)

    # with open(os.path.join("data/evaluation/threedod/arkitscenes", "arkitscenes_det_val_4frames.json")) as f:
    #     data = json.load(f)
