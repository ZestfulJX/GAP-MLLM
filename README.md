...
This repository provides the **GPU implementation**. The Ascend NPU implementation is maintained separately in the [NPU version](https://github.com/ZestfulJX/GAP-MLLM/tree/npu).
...
- [TODO] Release NPU training/inference code.
- [2026-07-27] Released checkpoints, data, and GPU training/inference code.
- [2026-06-18] Accepted by ECCV 2026.
...
3. **Install PyTorch for your CUDA environment first.**

    Install the `torch`, `torchvision`, and `torchaudio` builds that match your CUDA driver.
...
Before training or evaluation, download the required datasets and annotations according to the following folder structure.
...
```bash
python inference_vg_stagerefer.py \
    --ckpt_path ckpts/gapmllm-3d-3b \
    --data_path data/media \
...
python inference_recons.py \
    --ckpt_path ckpts/qwen3-gapmllm-pretraining-3b \
    --data_path data/media \
