...
def calculate_timestamps(n_images: int, fps: float = 1.0) -> List[str]:
    """Calculate timestamp strings for n_images, matching eval processor default behavior.

    In eval (simple/qwen3_vl.py), pre-sampled video frames get:
      frames_indices = [0, 1, ..., N-1], fps defaults to 24 (no real fps info).
    Each training image is one independent temporal unit (T=1), so timestamp for
    image i is simply i / fps, formatted as "<X.X seconds>".
    """
    return [f"<{i / fps:.1f} seconds>" for i in range(n_images)]
            ...
            if role == "user":
                ...
                if visual_tag in content:
                    ...
                    for i in range(len(parts) - 1):
                        ...
                        timestamp_prefix = timestamps[visual_replicate_index] if timestamps else ""
                        replacement = (
                            timestamp_prefix
                            + "<|vision_start|>"
                            + f"<|{visual_type}_pad|>"
...
class RelativeLazySupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        super(RelativeLazySupervisedDataset, self).__init__()

        dataset = data_args.dataset_use.strip().split(",")
        ...
        assert data_args.model_type == "qwen3vl", "We support other qwen version, but here we prefer qwen3vl for experiments"
    ...
    def visual_prompt(self, index):
        sources = self.list_data_dict[index]
        image_file = []
        if "pixel_coords" in sources.keys():
            pixel_coords = sources["pixel_coords"]
            images = sources["images"]
            view_idx, scaled_pixel_x, scaled_pixel_y = pixel_coords[0], pixel_coords[1], pixel_coords[2]
            for idx, img_path in enumerate(images):
                # 构建完整图片路径
                full_img_path = os.path.join(sources["data_path"], img_path)
                if not os.path.exists(full_img_path):
                    raise FileNotFoundError(f"图片不存在：{full_img_path}")
                # 打开并标记图片
                with Image.open(full_img_path) as img:
                    ...
                    if view_idx == idx:
                        cross_size = 10  # 叉号臂长
                        thickness = 3    # 线条粗细（像素数）
                        
                        # 绘制水平方向的粗线（包含上下偏移的多条线）
            ...
            if "images" in sources.keys():
                image_file = sources["images"]
            else:
                image_file = None
        return sources, image_file

    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        sources, image_file_dealed = self.visual_prompt(i)
        if isinstance(i, int):
            sources = [sources]
        ...
        if "image" in sources[0]:
            ...
            if image_file_dealed is not None:
                image_file = image_file_dealed # use the pasted images with visual marks
            else:
                image_file = self.list_data_dict[i]["image"]

            # print(str(len(image_file)) + '--' + image_file[0])
            if isinstance(image_file, List):
                ...
                # subsample frames if image_max_frames_selected is set, preserving original order
                # skip subsampling when spar_info contains frame-index-dependent markers
                image_max_frames_selected = getattr(self.data_args, "image_max_frames_selected", None)
                _spar_info = sources[0].get("spar_info", None)
                _has_frame_idx = False
                if _spar_info is not None:
                    _info = json.loads(_spar_info)
                    _has_frame_idx = "point_img_idx" in _info or "bbox_img_idx" in _info
                if image_max_frames_selected is not None and len(image_file) > image_max_frames_selected and not _has_frame_idx:
                    original_count = len(image_file)
                    indices = np.linspace(0, original_count - 1, image_max_frames_selected, dtype=int)
                    image_file = [image_file[idx] for idx in indices]
                    # sync <image> token count in conversation
                    for conv in sources[0]["conversations"]:
                        val = conv.get("value", conv.get("content", ""))
                        if DEFAULT_IMAGE_TOKEN in val:
                            new_val = val.replace(
                                DEFAULT_IMAGE_TOKEN * original_count,
                                DEFAULT_IMAGE_TOKEN * len(image_file),
                            )
                            if "value" in conv:
                                conv["value"] = new_val
                            else:
                                conv["content"] = new_val
                # draw visual markers
                self.draw_visual_marks(image_file, sources[0].get("spar_info", None))

                image, grid_thw, geometry_encoder_inputs, ori_images = [], [], [], []
            ...
            timestamps = calculate_timestamps(len(grid_thw))
        ...
        # data_dict["prompt"] = self.list_data_dict[i].get("prompt")
        # data_dict["conversations"] = self.list_data_dict[i].get("conversations")
        return data_dict
...
def make_supervised_data_module(
    ...
    train_dataset = RelativeLazySupervisedDataset(tokenizer=tokenizer, data_args=data_args)
