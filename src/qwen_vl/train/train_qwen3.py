...
from qwen_vl.data.data_qwen3_or import make_supervised_data_module
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if trainer.deepspeed:
        # 2
        # 获取当前NPU流并同步
        # stream = torch_npu.npu.current_stream()  # 获取当前设备的NPU流
        # stream.synchronize()  # 等待当前流上的所有NPU操作完成
        # trainer.save_model(output_dir)  # 同步完成后保存模型
        # return
...
def train(attn_implementation="flash_attention_2"):
    ...
    if "qwen3" in model_args.model_name_or_path.lower():
        ...
        raise ValueError(
            "qwen3 not in model_args.model_name_or_path.lower()"
        )
